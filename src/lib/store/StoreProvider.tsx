...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  // Persist on change (skip first render — else we'd write initial before hydration finishes)
  useEffect(() => {
  ...
  const actions: StoreActions = useMemo(() => ({
    ...
    approveStudent(id, teamId) {
      update(s => {
        ...
        if (!st) return {};
        // Simple demo access code — in prod this must be hashed on the server side before storage.
    ...
    setRegOpen(open) { update({ regOpen: open }); },
    ...
    setLogoDisplayMode(mode) { update({ logoDisplayMode: mode }); },
